...
const regexInterface = /export interface IOfferDetails \{[\s\S]*?\}/;
const newInterface = `export interface IOfferDetails {
  fullName: string;
  roleApplied: string;
...
code = code.replace(regexInterface, newInterface);

const regexCard = /doc\.setFillColor\(lightGrey\[0\], lightGrey\[1\], lightGrey\[2\]\);[\s\S]*?doc\.text\(details\.jobLocation \|\| "Visakhapatnam \/ Remote", 75, cardY \+ 56\);/;
const newCard = `doc.setFillColor(lightGrey[0], lightGrey[1], lightGrey[2]);
...
code = code.replace(regexCard, newCard);
...
console.log('Patched pdf.server.ts');

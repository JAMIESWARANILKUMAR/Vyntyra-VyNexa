...
pattern = r"if \(data\.status === \"submitted\"\).*?return \{ success: true \};\n  \}\);"
...
ops_content = re.sub(pattern, new_block, ops_content, flags=re.DOTALL)

with io.open(ops_file, "w", encoding="utf-8", newline="\n") as f:
    f.write(ops_content)

print("Patched operations.functions.ts with regex")

...
const regex = /export async function generateOfferLetterPDF[\s\S]*?(?=\nexport |\n$|$)/;

const newFunction = `export async function generateOfferLetterPDF(details: IOfferDetails): Promise<string> {
...
if (regex.test(code)) {
  ...
  console.log('Successfully updated to ultra premium design!');
} else {
  console.log('Failed to find generateOfferLetterPDF function.');
}

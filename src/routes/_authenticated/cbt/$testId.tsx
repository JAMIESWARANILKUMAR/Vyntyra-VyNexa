...
function CbtExamInterface() {
  ...
  return (
    <div className="flex h-screen bg-white font-sans select-none overflow-hidden">
      
      {/* Left Sidebar (Proctoring & Progress Panel) */}
      <div className="w-[320px] bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-300 flex flex-col shadow-2xl z-20 relative border-r border-slate-800">
        
        {/* Header Branding */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between z-30">
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-950">
          ...
          }} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black text-lg h-14 rounded-2xl shadow-[0_0_15px_rgba(16,185,129,0.2)] transition-colors">
            End Exam & Submit
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#F8FAFC] relative">
        ...
        {/* Top Header */}
        <div className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 z-20 shadow-sm relative">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-slate-800">Question {activeQ + 1}</h2>
          </div>
          
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 text-slate-400 font-medium text-xs bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200/50">
            ...
            <div className="flex flex-col items-end">
              ...
              <div className={`font-mono text-2xl font-black leading-tight ${ (timeLeft || 0) < 300 ? "text-rose-600 animate-pulse" : "text-slate-800" }`}>
        ...
        <div className="flex-1 overflow-y-auto p-16 pb-20 custom-scrollbar relative z-10">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-12">
              <h3 className="text-2xl text-slate-900 font-normal leading-relaxed whitespace-pre-wrap">{q?.question_text}</h3>
            </div>
            ...
            <div>
              {q?.question_type === "mcq" && q.options && (
                <div className="space-y-4">
                  {q.options.map((opt: any) => {
                    ...
                    return (
                      <label key={opt.id} onClick={() => setAnswers({...answers, [q.id]: opt})} className={`group flex items-center gap-5 p-6 rounded-xl cursor-pointer transition-all border shadow-sm hover:shadow-md ${
                        checked 
                        ...
                        <span className={`text-lg ${checked ? "text-emerald-950 font-semibold" : "text-slate-700 font-medium"}`}>{opt.text}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              {q?.question_type === "long_answer" && (
                <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-emerald-500/50 focus-within:border-emerald-500 transition-all">
                  <textarea 
            ...
            <div className="mt-16 pt-8 border-t border-slate-200 flex items-center justify-between">
              <Button 
                ...
                className="font-bold border-slate-200 text-slate-600 w-40 h-14 rounded-xl hover:bg-slate-50 text-lg transition-all"
              >
              ...
              <Button 
                ...
                className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold w-40 h-14 rounded-xl shadow-lg shadow-emerald-600/20 text-lg transition-all disabled:opacity-50"
              >

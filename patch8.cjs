...
code = code.replace(/\s*<Button size="sm" onClick=\{\(\) => setProvisionOpen\(true\)\} className="gap-2 text-xs shadow-sm">/, '\n' + replacement);

if (code.includes('Delete All Offer Letters')) {
    console.log('Success injecting via Regex!');
}
